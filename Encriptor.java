...
{
	Encriptor enc = new Encriptor('F');
	String text = "ABCDEFGHIJK LMNO ghghghg 5YZabcde ABC";
	System.out.println(text);
	String codedString = enc.codeCaeser(text);
	System.out.println(codedString);
	System.out.println(enc.decodeCaeser(codedString));
	//FileOutputStream fos = new FileOutputStream("code.dat");
	File f = new File("coded.txt");
	f.createNewFile();
	enc.writeBytes(enc.codeCaeser(text),f);
	System.out.println(f.length());
	String s = enc.readBytes(f);

	System.out.println(s);
	System.out.println("\n\n\n\n" + enc.decodeCaeser(s));

	System.out.println("Now decoding by substitution cypher\n\n\n");
	String key = "abefcdHGJIKLMNOPQRSTUVWxyz";
	String message  = "KHURAM IS A STRANGE 123 and MAN ZZ";
	System.out.println("The message is      " + message);
	System.out.println("The key is " + key);
	Encriptor enc2 = new Encriptor(key);
	String codemessage = enc2.codeSubstitutionCipher(message);
	System.out.println("The coded message is " + codemessage);
	String decodedmessage  = enc2.decodeSubstitutionCipher(codemessage);
	System.out.println("The decoded message is "+ decodedmessage);


		// System.out.println(enc2.codeTable);
		// System.out.println(enc2.decodeTable);
